    ...
    for group_idx in range(1, 16):
        ...
        for model_name in model_list:
            ...
            for train_index, test_index in skf.split(X, y):
                ...
                print(f"Training model: {model_name} for Group {group_idx}")
                if model_name == "xgboost":
                    model = make_xgb(scale_pos_weight = (num_negative / num_positive))
                elif model_name == "random_forest":
                    model = make_rf(scale_pos_weight = (num_negative / num_positive))
                elif model_name == "decision_tree":
                    model = make_dt(scale_pos_weight = (num_negative / num_positive))
                elif model_name == "logistic_regression":
                    model = make_lr(scale_pos_weight = (num_negative / num_positive))
                elif model_name == "svm":
                    model = make_svc(scale_pos_weight = (num_negative / num_positive))
                else:
            ...
            print(f"Group {group_idx}, Model {model_name}: Average Accuracy: {avg_acc}, Average AUC: {avg_auc}")
            print(f"Completed evaluation for Group {group_idx}, Model {model_name}.")

    # save performance records to a csv file
    performance_df = pd.DataFrame(performance_records)
    performance_df.to_csv(os.path.join(output_dir, "original_data_groupwise_model_performance_multiple_models.csv"), index=False)
    print("Saved groupwise model performance to original_data_groupwise_model_performance_multiple_models.csv")