...
def load_csv_data(csv_path, columns_def, group=None, target_col='malignancy', normalize_features=True):
    """
    ...
    Args:
        ...
        group: Group to filter by (if None, will use all groups)
...
if __name__ == "__main__":
    ...
    for group in Groups:
        ...
        if with_washout:
            save_fn = os.path.join(output_dir, "fusion_strategy_eval", f"{group}_groupwise_transformer_results_summary.csv")
        else:
            save_fn = os.path.join(output_dir, "fusion_strategy_eval", f"{group}_groupwise_transformer_results_summary_no_washout.csv")
